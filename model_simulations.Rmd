...
# holding vectors for equilibria (for bifurcation diagrams)
...
mtext(side = 1, expression("Strength of nutritional mutualism ("*b[g]*")"), line = 2.5)
...
mtext(side = 1, expression("Strength of defensive mutualism ("*b[cn]*")"), line = 2.5)
axis(side = 1, at = xaxs, las = 1)
...
pdf("figures/phi_opt_main_v4.pdf", width = 5.75, height = 5)
par(mfrow = c(2, 2))
layout(matrix(c(1, 2, 3, 4), nrow = 2, ncol = 2, byrow = F), widths = c(1, 1), heights = rep(1, 2))
#layout.show(5)
par(mar=c(0.25, 1, 0.5, 0), oma = c(4.5, 4.5, 1.75, 1))
...
mtext(side = 2, "Partner equilibrium (no.)", line = 3.5, cex = 1.1)
mtext(side = 3, "Nutritional mutualism")
...
legend(x = "topleft", legend = as.character(mut_test_set), col = c(adjustcolor(leg_col, alpha.f = alpha_set[1]), adjustcolor(leg_col, alpha.f = alpha_set[2]), adjustcolor(leg_col, alpha.f = alpha_set[3]), adjustcolor(leg_col, alpha.f = alpha_set[4]), adjustcolor(leg_col, alpha.f = alpha_set[5]), adjustcolor(leg_col, alpha.f = alpha_set[6])), bty = "n", lwd = 4, ncol = 2, cex = 0.75, inset = c(0.08, 0.15)) # , title = expression("Strength of partner's \neffects on host ("*b_g*")")
text(x = 0.32, y = 690, "Strength of partner's effect", cex = 0.8)
text(x = 0.32, y = 655, expression("on host growth ("*b[g]*")"), cex = 0.8)
...
mtext(side = 1, expression(Strength~of~partner~preference~'for'~adult~hosts~(phi)), line = 2.5, outer = T, cex = 1, adj = 0.4)
mtext(side = 2, expression(Host~equilibrium~(cm^2)), line = 3.5, cex = 1.1)
axis(side = 1, at = xaxt1)
...
mtext(side = 3, "Defensive mutualism")
axis(side = 1, at = xaxt1, labels = NA)
axis(side = 2, at = yaxt1, las = 1, hadj  = 0.75, labels = NA)
#abline(v = eco_opt_Dex$phi, lty = 1, col = Lcol)

leg_col <- Dcol

legend(x = "topright", legend = as.character(mut_test_set), col = c(adjustcolor(leg_col, alpha.f = alpha_set[1]), adjustcolor(leg_col, alpha.f = alpha_set[2]), adjustcolor(leg_col, alpha.f = alpha_set[3]), adjustcolor(leg_col, alpha.f = alpha_set[4]), adjustcolor(leg_col, alpha.f = alpha_set[5]), adjustcolor(leg_col, alpha.f = alpha_set[6])), bty = "n", lwd = 4, ncol = 2, cex = 0.75, inset = c(0.1, 0.4)) # , title = expression("Strength of partner's \neffects on host ("*b_g*")")
text(x = 0.68, y = 585, "Strength of partner's effect", cex = 0.8)
text(x = 0.68, y = 550, expression("on host survival ("*b[cn]*")"), cex = 0.8)
...
axis(side = 1, at = xaxt1)
axis(side = 2, at = yaxt2, las = 1, hadj = 0.75, labels = NA)
#abline(v = eco_opt_Dex$phi, lty = 1, col = Lcol)
...
#mtext(side = 3, "Nutritional mutualism")
mtext(side = 1, xlab, line = 2.5)
mtext(side = 2, expression(Predicted~strength~of~partner), line = 3.75, cex = 0.95)
mtext(side = 2, expression(preference~'for'~adult~hosts~(phi)), line = 2.5, cex = 0.95)
...
Also make a phase plane to show trajectories from different initial conditions:
...
# eco simulation
ecoNdef <- mod_fun3(tmax_test, P0 = P0.pp, H0 = H0.pp, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoNdef$eco_phi)

# evo simulation
evoNdef <- mod_fun3(tmax, P0 = P0.pp, H0 = H0.pp, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoNdef$evo_phi)


# repeat with different initial conditions
# P0.pp <- c(0.01, 0.01, 0.01)
# H0.pp <- c(10, 10, 10)

P0.pp2 <- c(0, 0, 300)
H0.pp2 <- c(0, 0, 2)

ecoNdef2 <- mod_fun3(tmax, P0 = P0.pp2, H0 = H0.pp2, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoNdef$eco_phi)

evoNdef2 <- mod_fun3(tmax, P0 = P0.pp2, H0 = H0.pp2, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoNdef$evo_phi)


P0.pp3 <- c(0, 1, 0)
H0.pp3 <- c(0, 50, 0)

ecoNdef3 <- mod_fun3(tmax, P0 = P0.pp3, H0 = H0.pp3, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoNdef$eco_phi)

evoNdef3 <- mod_fun3(tmax, P0 = P0.pp3, H0 = H0.pp3, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoNdef$evo_phi)
...
ecoDdef <- mod_fun3(tmax, P0 = P0.pp, H0 = H0.pp, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoDdef$eco_phi)

evoDdef <- mod_fun3(tmax, P0 = P0.pp, H0 = H0.pp, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoDdef$evo_phi)

# repeat with different initial conditions

ecoDdef2 <- mod_fun3(tmax, P0 = P0.pp2, H0 = H0.pp2, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoDdef$eco_phi)

evoDdef2 <- mod_fun3(tmax, P0 = P0.pp2, H0 = H0.pp2, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoDdef$evo_phi)


ecoDdef3 <- mod_fun3(tmax, P0 = P0.pp3, H0 = H0.pp3, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoDdef$eco_phi)

evoDdef3 <- mod_fun3(tmax, P0 = P0.pp3, H0 = H0.pp3, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoDdef$evo_phi)
...
H0_set1 <- seq(from = 1, to = 95, length.out = 20)
P0_set <- seq(from = 1, to = 700, length.out = 20)
...
evoD1_i <- list()
  ...
  for(j in 1:length(P0_set)){
    ...
    H0.ij <- rep(H0_set1[i]/3, 3)
    P0.ij <- rep(P0_set[j]/3, 3)

# eco simulation
ecoN_ij <- mod_fun3(t_short, P0 = P0.ij, H0 = H0.ij, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoNdef$eco_phi)

# evo simulation
evoN_ij <- mod_fun3(t_short, P0 = P0.ij, H0 = H0.ij, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoNdef$evo_phi)
...
ecoD_ij <- mod_fun3(t_short, P0 = P0.ij, H0 = H0.ij, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoDdef$eco_phi)

evoD_ij <- mod_fun3(t_short, P0 = P0.ij, H0 = H0.ij, rP, nP, mP_pars, eP, nH_pars.3, mH_pars, G_pars.3, F_pars, eH, Apatch, z, phi = ecoevoDdef$evo_phi)

# record the trajectories
ecoN1_i[[j]] <- matrix(c(apply(ecoN_ij$H*A_mids, MARGIN = 2, sum), apply(ecoN_ij$P, MARGIN = 2, sum)), nrow = t_short + 1, ncol = 2, byrow = F) # 1 = P, 2 = H
evoN1_i[[j]] <- matrix(c(apply(evoN_ij$H*A_mids, MARGIN = 2, sum), apply(evoN_ij$P, MARGIN = 2, sum)), nrow = t_short + 1, ncol = 2, byrow = F)
ecoD1_i[[j]] <- matrix(c(apply(ecoD_ij$H*A_mids, MARGIN = 2, sum), apply(ecoD_ij$P, MARGIN = 2, sum)), nrow = t_short + 1, ncol = 2, byrow = F)
evoD1_i[[j]] <- matrix(c(apply(evoD_ij$H*A_mids, MARGIN = 2, sum), apply(evoD_ij$P, MARGIN = 2, sum)), nrow = t_short + 1, ncol = 2, byrow = F)
...
#tend <- length(ecoNdef$t)

pdf("figures/phase_plane2.pdf", width = 7, height = 5)

par(mfrow = c(2, 2))
layout(matrix(c(1, 2, 3, 4), nrow = 2, ncol = 2, byrow = T), widths = rep(1, 2), heights = rep(1, 2))
#layout.show(12)
par(mar=c(0, 1.25, 1.5, 0.5), oma = c(4, 4.5, 1, 0.5))

# Neco
v_list <- ecoN1
plot(y = 1, x = 1, type = "l", ylim = c(0, 90000), xlim = c(0, 1150), col = NA, lwd=2, las = 1, xlab = NA, ylab = NA, xaxt = "n")
axis(side = 1, at = seq(from = 0, to = 1000, by = 200), labels = NA)
mtext(side = 3, expression("a) Nutritional, partner maximizing"~phi), adj = 0)
mtext(side = 1, "Total number of partners", line = 2.5, outer = T)
mtext(side = 2, expression("Total area covered by hosts ("*cm^2*")"), line = 2.25, outer = T)
# add vector field for the ecoN case
for(i in 1:length(H0_set1)){
  for(j in 1:length(P0_set)){
    lines(v_list[[i]][[j]][, 2], v_list[[i]][[j]][, 1], col = adjustcolor(Ncol, alpha.f = 0.5))
  }
}
# equilibria (closed circles)
points(y = apply(ecoNdef$H*A_mids, MARGIN = 2, sum)[tend], x = apply(ecoNdef$P, MARGIN = 2, sum)[tend], bg = Ncol, col = "black", pch = 21, cex = 1.2)
points(y = apply(evoNdef$H*A_mids, MARGIN = 2, sum)[tend], x = apply(evoNdef$P, MARGIN = 2, sum)[tend], bg = Ncol, col = "black", pch = 23, cex = 1.5)
points(y = apply(evoDdef$H*A_mids, MARGIN = 2, sum)[tend], x = apply(evoDdef$P, MARGIN = 2, sum)[tend], bg = Dcol, col = "black", pch = 23, cex = 1.6)
points(y = apply(ecoDdef$H*A_mids, MARGIN = 2, sum)[tend], x = apply(ecoDdef$P, MARGIN = 2, sum)[tend], bg = Dcol, col = "black", pch = 21, cex = 1.1)
legend("bottomright", title = "Equilibria", legend = c(expression("N, max"~phi), expression("N, ESS"~phi),expression("D, max"~phi), expression("D, ESS"~phi)), col = c("black", "black", "black", "black"), pch = c(21, 23, 21, 23), pt.bg = c(Ncol, Ncol, Dcol, Dcol),bty = "n", cex = 1, pt.cex = c(1.2, 1.4, 1.2, 1.4))

# arrows(x0, y0, x1 = x0, y1 = y0, length = 0.25, angle = 30,code = 2, col = par("fg"), lty = par("lty"),  lwd = par("lwd"), ...)

# Nevo
v_list <- evoN1
plot(y = 1, x = 1, type = "l", ylim = c(0, 90000), xlim = c(0, 1150), col = NA, lwd=2, las = 1, xlab = NA, ylab = NA, xaxt = "n", yaxt = "n")
axis(side = 1, at = seq(from = 0, to = 1000, by = 200), labels = NA)
axis(side = 2, at = seq(from = 0, to = 80000, by = 20000), labels = NA)
mtext(side = 3, expression("b) Nutritional, ESS"~phi), adj = 0)
mtext(side = 1, "Total number of partners", line = 2.5, outer = T)
mtext(side = 2, expression("Total area covered by hosts ("*cm^2*")"), line = 2.25, outer = T)
# add vector field for the ecoN case
for(i in 1:length(H0_set1)){
...
# Deco
v_list <- ecoD1
plot(y = 1, x = 1, type = "l", ylim = c(0, 90000), xlim = c(0, 1150), col = NA, lwd=2, las = 1, xlab = NA, ylab = NA)
mtext(side = 3, expression("c) Defensive, partner maximizing"~phi), adj = 0, line = -0.2)
mtext(side = 1, "Total number of partners", line = 2.5, outer = T)
mtext(side = 2, expression("Total area covered by hosts ("*cm^2*")"), line = 2.25, outer = T)
# add vector field for the ecoN case
for(i in 1:length(H0_set1)){
  for(j in 1:length(P0_set)){
    lines(v_list[[i]][[j]][, 2], v_list[[i]][[j]][, 1], col = adjustcolor(Dcol, alpha.f = 0.5))
  }
}
# equilibria (closed circles)
points(y = apply(ecoNdef$H*A_mids, MARGIN = 2, sum)[tend], x = apply(ecoNdef$P, MARGIN = 2, sum)[tend], bg = Ncol, col = "black", pch = 21, cex = 1.2)
...
# Devo
v_list <- evoD1
plot(y = 1, x = 1, type = "l", ylim = c(0, 90000), xlim = c(0, 1150), col = NA, lwd=2, las = 1, xlab = NA, ylab = NA, yaxt = "n")
axis(side = 2, at = seq(from = 0, to = 80000, by = 20000), labels = NA)
mtext(side = 3, expression("d) Defensive, ESS"~phi), adj = 0, line = -0.2)
mtext(side = 1, "Total number of partners", line = 2.5, outer = T)
# add vector field for the ecoN case
for(i in 1:length(H0_set1)){
  for(j in 1:length(P0_set)){
...
evo.p <- mod_fun3(tmax2, P0 = R0.0*p123, H0 = H0.0*p123, rP, nP, mP_pars, eP, nH_pars.4, mH_pars, G_pars.5, F_pars, eH, Apatch, z, phi = phi_evo)
eco.p <- mod_fun3(tmax2, P0 = R0.0*p123, H0 = H0.0*p123, rP, nP, mP_pars, eP, nH_pars.4, mH_pars, G_pars.5, F_pars, eH, Apatch, z, phi = phi_eco)

HA_evo <- apply(evo.p$H*A_mids, 2, sum)
HA_eco <- apply(eco.p$H*A_mids, 2, sum)

HA_evo.g0[i] <- HA_evo[tmax2]
HA_eco.g0[i] <- HA_eco[tmax2]

tr_eco.g0[i] <- min(which(abs((HA_eco-HA_eco[tmax2])) < 0.01*HA_eco[tmax2]))
tr_evo.g0[i] <- min(which(abs((HA_evo-HA_evo[tmax2])) < 0.01*HA_evo[tmax2]))

Ptot_evo <- apply(evo.p$P, 2, sum)
Ptot_eco <- apply(eco.p$P, 2, sum)
P_evo.g0[i] <- Ptot_evo[tmax2]
P_eco.g0[i] <- Ptot_eco[tmax2]

tr_eco.g0.P[i] <- min(which(abs((Ptot_eco-Ptot_eco[tmax2])) < 0.01*Ptot_eco[tmax2]))
tr_evo.g0.P[i] <- min(which(abs((Ptot_evo-Ptot_evo[tmax2])) < 0.01*Ptot_evo[tmax2]))
...
Fconst_set.1 <- seq(from = 0.1*Fconst, to = Fconst, length.out = length.2)
...
evo.p <- mod_fun3(tmax2, P0 = R0.0*p123, H0 = H0.0*p123, rP, nP, mP_pars, eP, nH_pars.4, mH_pars, G_pars.4, F_pars.5, eH, Apatch, z, phi = phi_evo)
eco.p <- mod_fun3(tmax2, P0 = R0.0*p123, H0 = H0.0*p123, rP, nP, mP_pars, eP, nH_pars.4, mH_pars, G_pars.4, F_pars.5, eH, Apatch, z, phi = phi_eco)

HA_evo <- apply(evo.p$H*A_mids, 2, sum)
HA_eco <- apply(eco.p$H*A_mids, 2, sum)

HA_evo.Fconst[i] <- HA_evo[tmax2]
HA_eco.Fconst[i] <- HA_eco[tmax2]

tr_eco.Fconst[i] <- min(which(abs((HA_eco-HA_eco[tmax2])) < 0.01*HA_eco[tmax2]))
tr_evo.Fconst[i] <- min(which(abs((HA_evo-HA_evo[tmax2])) < 0.01*HA_evo[tmax2]))

Ptot_evo <- apply(evo.p$P, 2, sum)
Ptot_eco <- apply(eco.p$P, 2, sum)
P_evo.Fconst[i] <- Ptot_evo[tmax2]
P_eco.Fconst[i] <- Ptot_eco[tmax2]

tr_eco.Fconst.P[i] <- min(which(abs((Ptot_eco-Ptot_eco[tmax2])) < 0.01*Ptot_eco[tmax2]))
tr_evo.Fconst.P[i] <- min(which(abs((Ptot_evo-Ptot_evo[tmax2])) < 0.01*Ptot_evo[tmax2]))
...
c_n0_set.1 <- seq(from = c_n0, to = 1.9*c_n0, length.out = length.2)
...
evo.p <- mod_fun3(tmax2, P0 = R0.0*p123, H0 = H0.0*p123, rP, nP, mP_pars, eP, nH_pars.5, mH_pars, G_pars.4, F_pars, eH, Apatch, z, phi = phi_evo)
eco.p <- mod_fun3(tmax2, P0 = R0.0*p123, H0 = H0.0*p123, rP, nP, mP_pars, eP, nH_pars.5, mH_pars, G_pars.4, F_pars, eH, Apatch, z, phi = phi_eco)

HA_evo <- apply(evo.p$H*A_mids, 2, sum)
HA_eco <- apply(eco.p$H*A_mids, 2, sum)

HA_evo.c_n0[i] <- HA_evo[tmax2]
HA_eco.c_n0[i] <- HA_eco[tmax2]

tr_eco.c_n0[i] <- min(which(abs((HA_eco-HA_eco[tmax2])) < 0.01*HA_eco[tmax2]))
tr_evo.c_n0[i] <- min(which(abs((HA_evo-HA_evo[tmax2])) < 0.01*HA_evo[tmax2]))

Ptot_evo <- apply(evo.p$P, 2, sum)
Ptot_eco <- apply(eco.p$P, 2, sum)
P_evo.c_n0[i] <- Ptot_evo[tmax2]
P_eco.c_n0[i] <- Ptot_eco[tmax2]

tr_eco.c_n0.P[i] <- min(which(abs((Ptot_eco-Ptot_eco[tmax2])) < 0.01*Ptot_eco[tmax2]))
tr_evo.c_n0.P[i] <- min(which(abs((Ptot_evo-Ptot_evo[tmax2])) < 0.01*Ptot_evo[tmax2]))
...
### Fig 5 and associated SM figures
```{r}
...
# 
# 
# xaxt.g0 <- seq(from = 0, to = 0.9, by = 0.2)
# xaxt.Fconst <- seq(from = 0, to = 0.9, by = 0.2)
# xaxt.c_n0 <- seq(from = 0, to = 0.9, by = 0.2)
# 
# 
# # replace recovery times for cases where the population dies with NA
# eco_indx <- which(HA_eco.Fconst < sum(H0.0*p123*A_mids)) # less than initial conditions
# evo_indx <- which(HA_eco.Fconst < sum(H0.0*p123*A_mids))
# 
# tr_evo.Fconst.Pp <- tr_evo.Fconst.P
# tr_eco.Fconst.Pp <- tr_eco.Fconst.P
# 
# tr_evo.Fconst.Pp[evo_indx] <- NA
# tr_eco.Fconst.Pp[eco_indx] <- NA
# 
...
# 
# 
# plot(x = rev(abs((g0_set.1-g0)/g0)), y = rev(tr_evo.g0.P), type = "l", lty = 2, ylim = c(0, ymax_tr), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Ncol)
# lines(x = rev(abs((g0_set.1-g0)/g0)), y = rev(tr_eco.g0.P), lty = 1, lwd = lwd.2, col = Ncol)
# axis(side = 1, at = xaxt.g0, labels = NA)
# axis(side = 2, at = yaxt2, las = 1)
# mtext(side = 3, expression("Decreased host growth rate ("*g[0]*")"), cex = 0.9)
# mtext(side = 2, "Recovery time (months)", cex = 0.9, line = 3)
# legend("topleft", legend = c("Partner maximizing", "Partner ESS"), lty = c(1, 2), lwd = lwd.2, bty = "n", col = Ncol, cex = 1.1)
# #abline(v = g0, lty = 3)
# 
# plot(x = rev(abs((Fconst_set.1 - Fconst)/Fconst)), y = rev(tr_evo.Fconst.Pp), type = "l", lty = 2, ylim = c(0, ymax_tr), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Ncol)
# axis(side = 1, at = xaxt.Fconst, labels = NA)
# axis(side = 2, at = yaxt2, labels = NA)
# lines(x = rev(abs((Fconst_set.1 - Fconst)/Fconst)), y = rev(tr_eco.Fconst.Pp), lty = 1, lwd = lwd.2, col = Ncol)
# mtext(side = 3, expression("Decreased host fecundity ("*F[const]*")"), cex = 0.9)
# #abline(v = Fconst, lty = 3)
# 
# plot(x = (c_n0_set.1-c_n0)/c_n0, y = tr_evo.c_n0.P, type = "l", lty = 2, ylim = c(0, ymax_tr), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Ncol)
# axis(side = 1, at = xaxt.c_n0, labels = NA)
# axis(side = 2, at = yaxt2, labels = NA)
# lines(x = (c_n0_set.1-c_n0)/c_n0, y = tr_eco.c_n0.P, lty = 1, lwd = lwd.2, col = Ncol)
# mtext(side = 3, expression("Increased host mortality ("*c[n0]*")"), cex = 0.9)
# #abline(v = c_n0, lty = 3)
# 
# ymax_HA <- 64000
# yaxt2 <- seq(from = 0, to = ymax_HA, by = 10000)
# 
# plot(x = rev(abs((g0_set.1-g0)/g0)), y = rev(HA_evo.g0), type = "l", lty = 2, ylim = c(0, ymax_HA), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Ncol)
# axis(side = 1, at = xaxt.g0, labels = xaxt.g0*100)
# axis(side = 2, at = yaxt2, las = 1)
# lines(x = rev(abs((g0_set.1-g0)/g0)), y = rev(HA_eco.g0), lty = 1, lwd = lwd.2, col = Ncol)
# #mtext(side = 1, "Post-disturbance \n baseline host growth rate", line = 3.5, cex = 0.8)
# #mtext(side = 1, expression(Post-disturbance~baseline), line = 2.25, cex = 0.8)
# #mtext(side = 1, expression(host~growth~rate~(month^-1)), line = 3.75, cex = 0.8)
# mtext(side = 1, "Strength of press disturbance (percent increase or decrease in parameter)", outer = T, line = 2.5) #adj = -0.2
# mtext(side = 2, expression(Host~equilibrium~(cm^2)), cex = 0.9, line = 3.25)
# #abline(v = g0, lty = 3)
# 
# plot(x = rev(abs((Fconst_set.1 - Fconst)/Fconst)), y = rev(HA_evo.Fconst), type = "l", lty = 2, ylim = c(0, ymax_HA), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Ncol)
# axis(side = 1, at = xaxt.Fconst, labels = xaxt.Fconst*100)
# axis(side = 2, at = yaxt2, labels = NA)
# lines(x = rev(abs((Fconst_set.1 - Fconst)/Fconst)), y = rev(HA_eco.Fconst), lty = 1, lwd = lwd.2, col = Ncol)
# #mtext(side = 1, "Post-disturbance \n host fecundity", line = 3.5, cex = 0.8)
# #mtext(side = 1, expression(Post-disturbance~host), line = 2.25, cex = 0.8)
# #mtext(side = 1, expression(fecundity~(month^-1)), line = 3.75, cex = 0.8)
# #abline(v = Fconst, lty = 3)
# 
# plot(x = (c_n0_set.1-c_n0)/c_n0, y = HA_evo.c_n0, type = "l", lty = 2, ylim = c(0, ymax_HA), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Ncol)
# axis(side = 1, at = xaxt.c_n0, labels = xaxt.c_n0*100)
# axis(side = 2, at = yaxt2, labels = NA)
# lines(x = (c_n0_set.1-c_n0)/c_n0, y = HA_eco.c_n0, lty = 1, lwd = lwd.2, col = Ncol)
# #mtext(side = 1, "Post-disturbance \n host DI mortality rate", line = 3.5, cex = 0.8)
# #mtext(side = 1, expression(Post-disturbance~host), line = 2.25, cex = 0.8)
# #mtext(side = 1, expression(DI~mortality~(month^-1)), line = 3.75, cex = 0.8)
# #abline(v = c_n0, lty = 3)
# 
# dev.off()
# 
...
# mtext(side = 1, expression(Post-disturbance~baseline), line = 2.25, cex = 0.8)
# mtext(side = 1, expression(host~growth~rate~(month^-1)), line = 3.75, cex = 0.8)
# mtext(side = 2, "Partner equilibrium (no.)", cex = 0.9, line = 3)
# legend("bottomright", legend = c("Partner maximizing", "Partner ESS"), lty = c(1, 2), lwd = lwd.2, bty = "n", col = Ncol, cex = 1.1)
# #abline(v = g0, lty = 3)
# 
...
combined version with the partners and hosts in the same figure
```{r}
...
pdf("figures/eco_evo_tr2.pdf", width = 7.5, height = 6)
...
ymax_tr <- tmax2
#yaxt2 <- seq(from = 0, to = ymax_tr, by = 250)
yaxt2 <- seq(from = 0, to = ymax_tr, by = 500)
...
plot(x = rev(abs((g0_set.1-g0)/g0)), y = rev(P_evo.g0), type = "l", lty = 2, ylim = c(0, ymax_P), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Ncol)
axis(side = 2, at = yaxt2, las = 1)
axis(side = 1, at = xaxt.g0, labels = xaxt.g0*100)
axis(side = 2, at = yaxt2, las = 1)
lines(x = rev(abs((g0_set.1-g0)/g0)), y = rev(P_eco.g0), lty = 1, lwd = lwd.2, col = Ncol)
mtext(side = 2, "Partner equilibrium (no.)", cex = 0.9, line = 3)

plot(x = rev(abs((Fconst_set.1 - Fconst)/Fconst)), y = rev(P_evo.Fconst), type = "l", lty = 2, ylim = c(0, ymax_P), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Ncol)
axis(side = 1, at = xaxt.Fconst, labels = xaxt.Fconst*100)
axis(side = 2, at = yaxt2, labels = NA)
lines(x = rev(abs((Fconst_set.1 - Fconst)/Fconst)), y = rev(P_eco.Fconst), lty = 1, lwd = lwd.2, col = Ncol)


plot(x = (c_n0_set.1-c_n0)/c_n0, y = P_evo.c_n0, type = "l", lty = 2, ylim = c(0, ymax_P), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Ncol)
axis(side = 1, at = xaxt.c_n0, labels = xaxt.c_n0*100)
axis(side = 2, at = yaxt2, labels = NA)
lines(x = (c_n0_set.1-c_n0)/c_n0, y = P_eco.c_n0, lty = 1, lwd = lwd.2, col = Ncol)
...
repeat for the defensive mutualism
...
ecoevoD_ts <- EcoEvo_fun(par_choice = "exp_cn", par_set = c(1), tmax, P0 = R0.0, H0 = H0.0, I0 = I0.1, rP, nP, mP_pars, eP, nH_pars.4, mH_pars, G_pars.4, F_pars, eH, Apatch, z, phi, phiRI)

max(ecoevoD_ts$eco_eq)
max(ecoevoD_ts$evo_eq)

phi_eco <- ecoevoD_ts$eco_phi
phi_evo <- ecoevoD_ts$evo_phi
...
pdf("figures/eco_evo_tr3.pdf", width = 7.5, height = 6)
...
plot(x = rev(abs((g0_set.1-g0)/g0)), y = rev(tr_evo.g0.P), type = "l", lty = 2, ylim = c(0, ymax_tr), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Dcol)
lines(x = rev(abs((g0_set.1-g0)/g0)), y = rev(tr_eco.g0.P), lty = 1, lwd = lwd.3, col = Dcol)
axis(side = 1, at = xaxt.g0, labels = NA)
axis(side = 2, at = yaxt2, las = 1)
mtext(side = 3, expression("Decreased host growth rate ("*g[0]*")"), cex = 0.9)
mtext(side = 2, "Recovery time (months)", cex = 0.9, line = 3)
legend("topleft", legend = c("Partner maximizing", "Partner ESS"), lty = c(1, 2), lwd = 2, bty = "n", col = Dcol, cex = 1.1)
#abline(v = g0, lty = 3)

plot(x = rev(abs((Fconst_set.1 - Fconst)/Fconst)), y = rev(tr_evo.Fconst.Pp), type = "l", lty = 2, ylim = c(0, ymax_tr), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Dcol)
axis(side = 1, at = xaxt.Fconst, labels = NA)
axis(side = 2, at = yaxt2, labels = NA)
lines(x = rev(abs((Fconst_set.1 - Fconst)/Fconst)), y = rev(tr_eco.Fconst.Pp), lty = 1, lwd = lwd.3, col = Dcol)
mtext(side = 3, expression("Decreased host fecundity ("*F[const]*")"), cex = 0.9)
#abline(v = Fconst, lty = 3)

plot(x = (c_n0_set.1-c_n0)/c_n0, y = tr_evo.c_n0.P, type = "l", lty = 2, ylim = c(0, ymax_tr), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Dcol)
axis(side = 1, at = xaxt.c_n0, labels = NA)
axis(side = 2, at = yaxt2, labels = NA)
lines(x = (c_n0_set.1-c_n0)/c_n0, y = tr_eco.c_n0.P, lty = 1, lwd = lwd.3, col = Dcol)
mtext(side = 3, expression("Increased host mortality ("*c[n0]*")"), cex = 0.9)
#abline(v = c_n0, lty = 3)

ymax_HA <- 100000
yaxt2 <- seq(from = 0, to = ymax_HA, by = 25000)

plot(x = rev(abs((g0_set.1-g0)/g0)), y = rev(HA_evo.g0), type = "l", lty = 2, ylim = c(0, ymax_HA), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Dcol)
axis(side = 1, at = xaxt.g0, labels = NA)
axis(side = 2, at = yaxt2, las = 1)
lines(x = rev(abs((g0_set.1-g0)/g0)), y = rev(HA_eco.g0), lty = 1, lwd = lwd.3, col = Dcol)
#mtext(side = 1, "Post-disturbance \n baseline host growth rate", line = 3.5, cex = 0.8)
#mtext(side = 1, expression(Post-disturbance~baseline), line = 2.25, cex = 0.8)
#mtext(side = 1, expression(host~growth~rate~(month^-1)), line = 3.75, cex = 0.8)
mtext(side = 1, "Strength of press disturbance (percent increase or decrease in parameter)", outer = T, line = 2.5) #adj = -0.2
...
plot(x = rev(abs((Fconst_set.1 - Fconst)/Fconst)), y = rev(HA_evo.Fconst), type = "l", lty = 2, ylim = c(0, ymax_HA), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Dcol)
axis(side = 1, at = xaxt.Fconst, labels = NA)
axis(side = 2, at = yaxt2, labels = NA)
lines(x = rev(abs((Fconst_set.1 - Fconst)/Fconst)), y = rev(HA_eco.Fconst), lty = 1, lwd = lwd.3, col = Dcol)
#mtext(side = 1, "Post-disturbance \n host fecundity", line = 3.5, cex = 0.8)
#mtext(side = 1, expression(Post-disturbance~host), line = 2.25, cex = 0.8)
#mtext(side = 1, expression(fecundity~(month^-1)), line = 3.75, cex = 0.8)
#abline(v = Fconst, lty = 3)

plot(x = (c_n0_set.1-c_n0)/c_n0, y = HA_evo.c_n0, type = "l", lty = 2, ylim = c(0, ymax_HA), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Dcol)
axis(side = 1, at = xaxt.c_n0, labels = NA)
axis(side = 2, at = yaxt2, labels = NA)
lines(x = (c_n0_set.1-c_n0)/c_n0, y = HA_eco.c_n0, lty = 1, lwd = lwd.3, col = Dcol)
#mtext(side = 1, "Post-disturbance \n host DI mortality rate", line = 3.5, cex = 0.8)
#mtext(side = 1, expression(Post-disturbance~host), line = 2.25, cex = 0.8)
#mtext(side = 1, expression(DI~mortality~(month^-1)), line = 3.75, cex = 0.8)
#abline(v = c_n0, lty = 3)
...
plot(x = rev(abs((g0_set.1-g0)/g0)), y = rev(P_evo.g0), type = "l", lty = 2, ylim = c(0, ymax_P), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Dcol)
axis(side = 2, at = yaxt2, las = 1)
axis(side = 1, at = xaxt.g0, labels = xaxt.g0*100)
axis(side = 2, at = yaxt2, las = 1)
lines(x = rev(abs((g0_set.1-g0)/g0)), y = rev(P_eco.g0), lty = 1, lwd = lwd.3, col = Dcol)
mtext(side = 2, "Partner equilibrium (no.)", cex = 0.9, line = 3)

plot(x = rev(abs((Fconst_set.1 - Fconst)/Fconst)), y = rev(P_evo.Fconst), type = "l", lty = 2, ylim = c(0, ymax_P), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Dcol)
axis(side = 1, at = xaxt.Fconst, labels = xaxt.Fconst*100)
axis(side = 2, at = yaxt2, labels = NA)
lines(x = rev(abs((Fconst_set.1 - Fconst)/Fconst)), y = rev(P_eco.Fconst), lty = 1, lwd = lwd.3, col = Dcol)


plot(x = (c_n0_set.1-c_n0)/c_n0, y = P_evo.c_n0, type = "l", lty = 2, ylim = c(0, ymax_P), xlab = NA, ylab = NA, xaxt = "n", yaxt = "n", lwd = lwd.2, col = Dcol)
axis(side = 1, at = xaxt.c_n0, labels = xaxt.c_n0*100)
axis(side = 2, at = yaxt2, labels = NA)
lines(x = (c_n0_set.1-c_n0)/c_n0, y = P_eco.c_n0, lty = 1, lwd = lwd.3, col = Dcol)
...
mtext(side = 2, line = 1.5, "% change in number of hosts", outer = T)
...
mtext(side = 3, line = -1.5, expression(j*")"~A[rep]), adj = 0.05)
lines(x = p_x, y = (psim$Heq1[ ,4]-dH1eq[2])/dH1eq[2]*100, lty = 2, lwd = 2)
lines(x = p_x, y = (psim$Heq1[ ,5]-dH1eq[3])/dH1eq[3]*100, lty = 1, lwd = 2)
...
legend("left", c("1", "2", "3"), lty = c(3, 2, 1, 1), col = c("black", "black", "black"), ncol = 1, lwd = 2, title = "Size class", bty = "n", cex = 1.2)
